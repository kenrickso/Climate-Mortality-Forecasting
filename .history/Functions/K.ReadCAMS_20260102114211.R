...
DatasetAggregator <- R6::R6Class("DatasetAggregator",
  public = list(
    dir = NULL,
    pattern = NULL,
    date_col = NULL,
    ...
    files = NULL,
    aggregated = NULL,
C:\1 My Code\FullDataset\Data
    initialize = function(dir = "C:\1 My Code\FullDataset\DataData/CAMS",
                          pattern = "\\.txt$",
                          date_col = "Date",
                          region_col = "Region",
      ...
      if (is.null(read_fun)) {
        self$read_fun <- if (requireNamespace("data.table", quietly = TRUE)) data.table::fread else utils::read.csv
      } else {
    ...
    refresh_files = function() {
      self$files <- list.files(self$dir, pattern = self$pattern, full.names = TRUE)
    ...
    head_all_files = function(n = 6, read_fun = NULL, ...) {
      ...
      files <- self$files
      if (length(files) == 0) stop("No files to show. Call refresh_files() or check dir.")
      out <- list()
      for (f in files) {
        cat("\n---", basename(f), "---\n")
        df_head <- tryCatch(
    ...
    read_and_agg_file = function(file, date_col = NULL, region_col = NULL, agg_fun = NULL, na.rm = NULL, read_fun = NULL, ...) {
      ...
      df <- tryCatch(
        ...
        error = function(e) stop("Read failed for ", basename(file), ": ", conditionMessage(e))
      )
      df <- as.data.frame(df, stringsAsFactors = FALSE)

      if (!date_col %in% names(df)) stop("Date column '", date_col, "' not found in ", basename(file))
      parsed <- as.POSIXct(df[[date_col]], tz = "UTC", tryFormats = c("%Y-%m-%d %H:%M:%S", "%Y-%m-%d"))
      if (all(is.na(parsed))) parsed <- as.Date(df[[date_col]])
      ...
      num_cols <- setdiff(names(df)[sapply(df, is.numeric)], c(date_col, region_col))
      if (length(num_cols) == 0) stop("No numeric columns to aggregate in ", basename(file))

      # use the actual column names provided by the user for grouping (no hard-coded "Region"/"Date")
      agg_by <- setNames(list(df[[region_col]], df[[date_col]]), c(region_col, date_col))
      agg_df <- aggregate(df[num_cols], by = agg_by, FUN = function(x) agg_fun(x, na.rm = na.rm))
    ...
    aggregate_files = function(files = NULL, date_col = NULL, region_col = NULL, agg_fun = NULL, na.rm = NULL, read_fun = NULL, ...) {
      date_col <- date_col %||% self$date_col
      region_col <- region_col %||% self$region_col
      agg_fun <- agg_fun %||% self$agg_fun
      na.rm <- na.rm %||% self$na.rm
      ...
      if (length(files) == 0) stop("No files matching pattern in dir: ", self$dir)

      lst <- lapply(files, function(f) {
        tryCatch(
          self$read_and_agg_file(f, date_col = date_col, region_col = region_col, agg_fun = agg_fun, na.rm = na.rm, read_fun = read_fun, ...),
          error = function(e) { message("Skipping ", basename(f), ": ", conditionMessage(e)); NULL }
        )
      })
      lst <- Filter(Negate(is.null), lst)
      if (length(lst) == 0) stop("No files could be read and aggregated.")

      lst <- lapply(lst, function(x) {
        x <- as.data.frame(x, stringsAsFactors = FALSE)
        if (!(region_col %in% names(x))) x[[region_col]] <- NA_character_
      ...
      aggregated_df <- Reduce(function(a, b) merge(a, b, by = by_cols, all = TRUE), lst)  # generic name

      aggregated_df[[region_col]] <- as.character(aggregated_df[[region_col]])
      aggregated_df[[date_col]] <- as.Date(aggregated_df[[date_col]])
      aggregated_df <- aggregated_df[order(aggregated_df[[region_col]], aggregated_df[[date_col]]), , drop = FALSE]
    ...
    select_avg_cols = function(pattern = "avg", ignore.case = TRUE) {
      ...
      keep <- c(self$region_col, self$date_col, grep(pattern, names(df), value = TRUE, ignore.case = ignore.case))
