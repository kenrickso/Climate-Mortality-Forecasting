...
setwd("C:/1 My Code/FullDataset/Code/Functions")
...
################################################################
#### Forecast yearly kappa2 for next year using Random Walk ####
################################################################
...
################################################################
######## Forecast weekly mortality rates for next year #########
################################################################
...
year_forecast <- year_last_fit # Change to forecast for 2019 with uncertainty
...
cnn_mort_fc_net <- function(
    ...
    if (cell_type == "gru") {
        x <- x %>%
            layer_gru(units = tail(cnn_filters, 1), return_sequences = FALSE)
    } else {
        x <- x %>%
            layer_lstm(units = tail(cnn_filters, 1), return_sequences = FALSE)
    }
...
run_variant <- function(cell_type, epochs = 80) {
    mdl <- cnn_mort_fc_net(
        seq_length = seq_length,
...
# Run both variants (use fewer epochs for quick comparison; adjust `epochs_compare` as needed)
variants <- c("gru", "lstm")
epochs_compare <- 80
results <- lapply(variants, function(v) run_variant(v, epochs = epochs_compare))
...
variant_means <- lapply(results, function(res) {
    train_mean <- compute_mean_logmxt(
        ...
        paste0("Adjusted Train (", display_cell_name(res$cell_type), ")")
    )
    test_mean <- compute_mean_logmxt(
        res$adjusted_test_wide,
        paste0("Adjusted Test (", display_cell_name(res$cell_type), ")")
    )
...
plot_data <- dplyr::bind_rows(
    observed_mean,
    baseline_2019_mean,
    variant_means[[1]]$train,
    variant_means[[1]]$test,
    variant_means[[2]]$train,
    variant_means[[2]]$test
) %>%
...
## Build combined plot for observed, baseline, CNN-GRU and CNN-LSTM variants
windows()
...
ggplot(
    ...
    labs(
        title = "Mean Log Mortality: Observed vs Baseline vs CNN-GRU/CNN-LSTM Adjusted",
...
get_result_by_type <- function(results_list, cell) {
    for (r in results_list) {
        if (!is.null(r$cell_type) && tolower(r$cell_type) == tolower(cell)) {
            return(r)
        }
    }
    NULL
...
res_gru <- get_result_by_type(results, "gru")
res_lstm <- get_result_by_type(results, "lstm")
...
gru_train_specific <- if (!is.null(res_gru)) {
    get_specific_from_adjusted(
        ...
        paste0("Adjusted Train (", display_cell_name(res_gru$cell_type), ")"),
        specific_col
    )
} else {
    NULL
...
gru_test_specific <- if (!is.null(res_gru)) {
    get_specific_from_adjusted(
        ...
        paste0("Adjusted Test (", display_cell_name(res_gru$cell_type), ")"),
        specific_col
    )
} else {
    NULL
...
lstm_train_specific <- if (!is.null(res_lstm)) {
    get_specific_from_adjusted(
        ...
        paste0("Adjusted Train (", display_cell_name(res_lstm$cell_type), ")"),
        specific_col
    )
} else {
    NULL
...
lstm_test_specific <- if (!is.null(res_lstm)) {
    get_specific_from_adjusted(
        ...
        paste0("Adjusted Test (", display_cell_name(res_lstm$cell_type), ")"),
        specific_col
    )
} else {
    NULL
...
compute_mae <- function(pred_wide, obs_wide) {
    if (is.null(pred_wide) || is.null(obs_wide)) {
        return(list(mae_log = NA_real_, mae_mxt = NA_real_))
    }
    ...
    # Compute absolute errors on log scale and original scale
    diff_log <- abs(pred_mat - obs_mat)
    mae_log <- mean(diff_log[is.finite(diff_log)], na.rm = TRUE)

    diff_mxt <- abs(exp(pred_mat) - exp(obs_mat))
    mae_mxt <- mean(diff_mxt[is.finite(diff_mxt)], na.rm = TRUE)

    list(mae_log = mae_log, mae_mxt = mae_mxt)
}

# Calculate for both variants and save results
mae_rows <- list()
for (res in results) {
    cell <- toupper(res$cell_type)
    mae_test <- compute_mae(res$adjusted_test_wide, observed_logmxt_wide)
    mae_train <- compute_mae(res$adjusted_train_wide, observed_logmxt_wide)
    msg <- sprintf(
        "%s MAE (log): test=%.6g, train=%.6g; MAE (mxt): test=%.6g, train=%.6g",
        cell,
        mae_test$mae_log,
        mae_train$mae_log,
        mae_test$mae_mxt,
    ...
    cat(msg, "\n")
    mae_rows[[cell]] <- data.frame(
        cell_type = cell,
        mae_log_test = mae_test$mae_log,
        mae_log_train = mae_train$mae_log,
        mae_mxt_test = mae_test$mae_mxt,
        mae_mxt_train = mae_train$mae_mxt
    )
}

# Calculate MAE for baseline (Lee-Carter)
mae_baseline_test <- compute_mae(baseline_logmxt_wide, observed_logmxt_wide)
mae_baseline_train <- compute_mae(
    baseline_logmxt_wide_2013_2018,
    observed_logmxt_wide
)
msg_baseline <- sprintf(
    "BASELINE MAE (log): test=%.6g, train=%.6g; MAE (mxt): test=%.6g, train=%.6g",
    mae_baseline_test$mae_log,
    mae_baseline_train$mae_log,
    mae_baseline_test$mae_mxt,
    mae_baseline_train$mae_mxt
)
cat(msg_baseline, "\n")
mae_rows[["BASELINE"]] <- data.frame(
    cell_type = "BASELINE",
    mae_log_test = mae_baseline_test$mae_log,
    mae_log_train = mae_baseline_train$mae_log,
    mae_mxt_test = mae_baseline_test$mae_mxt,
    mae_mxt_train = mae_baseline_train$mae_mxt
)

mae_table <- do.call(rbind, mae_rows)
mae_table[, 2:5] <- round(mae_table[, 2:5], 4)
mae_table

# Compute MAE per age group
ages <- c("67", "72", "77", "82", "87", "92")

compute_mae_per_age <- function(pred_wide, obs_wide, age) {
    ...
    compute_mae(pred_sub, obs_sub)
}

# MAE per age for each model
mae_per_age_rows <- list()
for (age in c("67", "72", "77", "82", "87", "92")) {
    for (res in results) {
        cell <- toupper(res$cell_type)
        mae_test <- compute_mae_per_age(
            res$adjusted_test_wide,
            observed_logmxt_wide,
            age
        )
        mae_train <- compute_mae_per_age(
            res$adjusted_train_wide,
            observed_logmxt_wide,
            age
        )
        mae_per_age_rows[[paste(age, cell, sep = "_")]] <- data.frame(
            age = age,
            cell_type = cell,
            mae_log_test = mae_test$mae_log,
            mae_log_train = mae_train$mae_log,
    ...
    # Baseline
    mae_baseline_test <- compute_mae_per_age(
        baseline_logmxt_wide,
        observed_logmxt_wide,
        age
    )
    mae_baseline_train <- compute_mae_per_age(
        baseline_logmxt_wide_2013_2018,
        observed_logmxt_wide,
        age
    )
    mae_per_age_rows[[paste(age, "BASELINE", sep = "_")]] <- data.frame(
        age = age,
        cell_type = "BASELINE",
        mae_log_test = mae_baseline_test$mae_log,
        mae_log_train = mae_baseline_train$mae_log,
...
mae_per_age_table <- do.call(rbind, mae_per_age_rows)
mae_per_age_table[, 3:6] <- round(mae_per_age_table[, 3:6], 4)
mae_per_age_table
...
plot_data_per_age <- dplyr::bind_rows(
    observed_per_age,
    baseline_per_age,
    variant_per_age[[1]]$train,
    variant_per_age[[1]]$test,
    variant_per_age[[2]]$train,
    variant_per_age[[2]]$test
) %>%
...
ggplot(
    ...
    scale_x_date(limits = as.Date(c("2013-01-01", "2019-12-31"))) +
    labs(
        title = "Mean Log Mortality per Age Group: Observed vs Baseline vs CNN-GRU/CNN-LSTM Adjusted",
        subtitle = age_map_note,
        x = "Week",
        y = "Mean Log Mxt",
        color = "Series"
...
# --- Monte Carlo LSTM Path Generation ---
# Generate 100 possible forecast paths for LSTM using empirical residuals

# Get the LSTM model and results
res_lstm <- results[[which(variants == "lstm")]]
# Assuming the model is stored in the environment or we need to retrain
# For simplicity, retrain the LSTM model to get mdl
mdl <- cnn_mort_fc_net(
    seq_length = seq_length,
    input_size = n_features,
    cnn_filters = c(128, 64, 32),
...
# Compute in-sample residuals
pred_train_z <- predict(mdl, X_train)
pred_train <- unscale_targets(pred_train_z, center = y_center, scale = y_scale)
actual_train <- unscale_targets(y_train, center = y_center, scale = y_scale)
residuals <- actual_train - pred_train # matrix: weeks x age groups

# Forecast 52 weeks ahead (point forecast)
point_forecast_z <- predict(mdl, seqs_forecast$X)
point_forecast <- unscale_targets(
    point_forecast_z,
    center = y_center,
    scale = y_scale
)

# Monte Carlo simulation: generate 100 sample paths
set.seed(123)
n_paths <- 100
n_steps <- 52
n_age_groups <- ncol(point_forecast) # number of age groups

mc_paths <- array(NA, dim = c(n_steps, n_age_groups, n_paths))

for (i in 1:n_paths) {
    for (t in 1:n_steps) {
        for (a in 1:n_age_groups) {
            noise <- sample(residuals[, a], 1, replace = TRUE)
            mc_paths[t, a, i] <- point_forecast[t, a] + noise
        }
    }
}

# Plot the sample paths for one age group (e.g., first age group)
age_idx <- 2
matplot(
    mc_paths[, age_idx, ],
    type = 'l',
    ...
    ylab = 'Forecast',
    main = paste(
        'Monte Carlo LSTM Forecast Paths for Age Group',
        colnames(point_forecast)[age_idx]
    )
)
lines(point_forecast[, age_idx], col = 'red', lwd = 2)
# --- End Monte Carlo LSTM Path Generation ---

# --- Generate 100 Lee-Carter Forecasts for 2019 with Kappa Uncertainty ---
kappa2_forecast_1y <- forecast_kappa2_random_walk_1y(
    kappa2_mat = kappa2_mat,
    year_last = year_last_fit,
    n_sim = 100, # Reduced for speed
    level = 0.95
)
## Extract simulated kappa2 matrix; fail with diagnostics if missing
kappa2_sim <- NULL
if (
    !is.null(kappa2_forecast_1y) &&
        is.list(kappa2_forecast_1y) &&
        "kappa2_sim" %in% names(kappa2_forecast_1y)
) {
    kappa2_sim <- kappa2_forecast_1y$kappa2_sim
} else if (is.data.frame(kappa2_forecast_1y)) {
    # The function returned a summary data.frame with per-region moments (mean +/- CI)
    df_k <- kappa2_forecast_1y
    if (!("region" %in% names(df_k)) || !("kappa2_mean" %in% names(df_k))) {
        cat(
            "--- DEBUG: forecast_kappa2_random_walk_1y returned data.frame columns:\n"
        )
        print(names(df_k))
        stop(
            "Unexpected structure in kappa2 forecast summary; need 'region' and 'kappa2_mean' columns"
        )
    }
    sim_regions <- as.character(df_k$region)
    n_sim <- 10
    kappa2_sim_mat <- matrix(NA_real_, nrow = length(sim_regions), ncol = n_sim)
    rownames(kappa2_sim_mat) <- sim_regions
    has_ci <- all(c("kappa2_lower", "kappa2_upper") %in% names(df_k))
    for (r in seq_along(sim_regions)) {
        mu <- as.numeric(df_k$kappa2_mean[r])
        if (has_ci) {
            lo <- as.numeric(df_k$kappa2_lower[r])
            hi <- as.numeric(df_k$kappa2_upper[r])
            sd_est <- (hi - lo) / (2 * qnorm(0.975))
            if (!is.finite(sd_est) || sd_est <= 0) {
                sd_est <- max(1e-4, abs(mu) * 0.01)
            }
        } else {
            sd_est <- max(1e-4, abs(mu) * 0.01)
        }
        kappa2_sim_mat[r, ] <- stats::rnorm(n_sim, mean = mu, sd = sd_est)
    }
    kappa2_sim <- kappa2_sim_mat
} else {
    cat("--- DEBUG: forecast_kappa2_random_walk_1y output keys:\n")
    if (is.null(kappa2_forecast_1y)) {
        cat("kappa2_forecast_1y is NULL\n")
    } else {
        print(names(kappa2_forecast_1y))
        str(kappa2_forecast_1y, max.level = 1)
    }
    stop(
        "kappa2_sim not found and cannot construct simulations from output; cannot proceed"
    )
}

# Diagnostics and robust alignment of region names
if (!is.matrix(kappa2_sim)) {
    kappa2_sim <- as.matrix(kappa2_sim)
}
sim_regions <- rownames(kappa2_sim)
if (is.null(sim_regions)) {
    sim_regions <- as.character(seq_len(nrow(kappa2_sim)))
}

## Derive region names from `lca` components (beta1/kappa2) or fallback to fitted regions
lca_regions <- NULL
if (!is.null(lca$beta1) && !is.null(colnames(lca$beta1))) {
    lca_regions <- colnames(lca$beta1)
} else if (!is.null(colnames(kappa2_mat))) {
    lca_regions <- colnames(kappa2_mat)
} else if (exists("regions_fit")) {
    lca_regions <- regions_fit
}
if (is.null(lca_regions)) {
    stop(
        "Could not determine region names from the Lee-Carter fit; cannot align regions"
    )
}

# Try to match exact names first
idx <- match(lca_regions, sim_regions)
if (any(is.na(idx))) {
    # Try coarse match by first 4 characters (NUTS code prefix)
    sim_short <- substr(sim_regions, 1, 4)
    target_short <- substr(lca_regions, 1, 4)
    idx2 <- match(target_short, sim_short)
    if (all(is.na(idx2))) {
        stop(
            "Cannot align region names between LC fit regions and kappa2_sim rownames.\n",
            "Available sim regions: ",
            paste(head(sim_regions, 10), collapse = ", ")
        )
    }
    idx[is.na(idx)] <- idx2[is.na(idx)]
}

# Reorder kappa2_sim rows to match LC fit regions
kappa2_sim <- kappa2_sim[idx, , drop = FALSE]

# Prepare LC forecast list
lc_forecasts <- list()
for (i in seq_len(ncol(kappa2_sim))) {
    # Extract numeric vector and name it with LC fit regions
    kappa2_next_mean <- as.numeric(kappa2_sim[, i])
    names(kappa2_next_mean) <- lca_regions
    # Quick sanity check before calling forecast function
    if (!is.numeric(kappa2_next_mean) || is.null(names(kappa2_next_mean))) {
        stop(
            "kappa2_next_mean must be a named numeric vector (names = regions)"
        )
    }
    # Optional diagnostic (prints first few names)
    cat(sprintf(
        "[LC] sim=%d, regions sample: %s\n",
        i,
        paste(
            names(kappa2_next_mean)[1:min(5, length(kappa2_next_mean))],
            collapse = ","
        )
    ))
    mortality_rate_forecast_1y <- forecast_weekly_mortality_rate_1y(
        lca = lca,
        kappa2_next_mean = kappa2_next_mean,
        year_forecast = year_forecast,
        iso_weeks = 1:52
    )
    # Get wide table for log mxt
    forecast_wide <- mortality_rate_forecast_1y$forecast_long %>%
        dplyr::select(Week, AgeNumeric, Region, logmxt_forecast) %>%
        tidyr::pivot_wider(
            id_cols = Week,
            names_from = c(AgeNumeric, Region),
        ...
        dplyr::arrange(Week)
    lc_forecasts[[i]] <- forecast_wide
}

# Compute mean log mxt for each LC forecast (weeks x n_lc)
lc_means <- sapply(lc_forecasts, function(df) rowMeans(df[, -1], na.rm = TRUE))

# Ensure Monte Carlo LSTM paths exist; generate if missing
if (!exists("mc_paths")) {
    # Build per-age residual matrix to sample from
    if (exists("residuals") && is.matrix(residuals)) {
        residuals_age_mat <- residuals
    } else if (exists("residuals_matrix") && is.matrix(residuals_matrix)) {
        col_ages <- sub("_.*", "", colnames(residuals_matrix))
        age_groups <- unique(col_ages)
        residuals_age_mat <- sapply(age_groups, function(a) {
            rowMeans(
                residuals_matrix[, col_ages == a, drop = FALSE],
                na.rm = TRUE
            )
        })
        residuals_age_mat <- as.matrix(residuals_age_mat)
        colnames(residuals_age_mat) <- age_groups
    } else {
        stop(
            "No residuals available to sample from (need 'residuals' or 'residuals_matrix')"
        )
    }

    # Determine point forecast matrix; if missing, use zeros (i.e., only add sampled residuals)
    if (exists("point_forecast") && is.matrix(point_forecast)) {
        pf <- point_forecast
    } else {
        n_steps_pf <- 52
        pf <- matrix(0, nrow = n_steps_pf, ncol = ncol(residuals_age_mat))
        colnames(pf) <- colnames(residuals_age_mat)
    }

    # Ensure pf has appropriate rows
    n_steps <- nrow(pf)
    if (nrow(residuals_age_mat) < 1) {
        stop("Residuals matrix has no rows to sample from")
    }

    set.seed(123)
    n_paths <- 10 # reduce for quick runs; increase to 100 for final analysis
    n_age_groups <- ncol(residuals_age_mat)
    mc_paths <- array(NA_real_, dim = c(n_steps, n_age_groups, n_paths))
    for (i in seq_len(n_paths)) {
        for (t in seq_len(n_steps)) {
            # sample a historical row index to draw residuals for all ages
            r_idx <- sample(seq_len(nrow(residuals_age_mat)), 1, replace = TRUE)
            mc_paths[t, , i] <- pf[t, ] + residuals_age_mat[r_idx, ]
        }
    }
}

# LSTM MC: compute mean across age groups for each week and each MC path
mc_paths_mean_all <- apply(mc_paths, c(1, 3), mean, na.rm = TRUE) # weeks x n_mc
print(sprintf("[CHECK] LSTM MC weeks: %d", nrow(mc_paths_mean_all)))

# Align rows
n_rows_lc <- nrow(lc_means)
n_rows_mc <- nrow(mc_paths_mean_all)
if (n_rows_lc != n_rows_mc) {
    warning(sprintf(
        "Row count mismatch: LC=%d, LSTM_MC=%d. Aligning by truncation to min=%d.",
        n_rows_lc,
        n_rows_mc,
        min(n_rows_lc, n_rows_mc)
    ))
    nmin <- min(n_rows_lc, n_rows_mc)
    lc_means <- lc_means[seq_len(nmin), , drop = FALSE]
    mc_paths_mean_all <- mc_paths_mean_all[seq_len(nmin), , drop = FALSE]
    # Also truncate forecast weeks if present
    if (exists("forecast_weeks") && length(forecast_weeks) >= nmin) {
        forecast_weeks <- forecast_weeks[seq_len(nmin)]
    } else if (exists("lc_forecasts") && length(lc_forecasts) > 0) {
        if (nrow(lc_forecasts[[1]]) >= nmin) {
            forecast_weeks <- lc_forecasts[[1]]$Week[seq_len(nmin)]
        }
    }
}

# Align number of MC columns to number of LC sims
n_lc <- ncol(lc_means)
n_mc <- ncol(mc_paths_mean_all)
if (n_mc == 0) {
    stop("No Monte Carlo LSTM paths available (mc_paths)")
}
if (n_mc >= n_lc) {
    sel <- round(seq(1, n_mc, length.out = n_lc))
    mc_paths_mean <- mc_paths_mean_all[, sel, drop = FALSE]
} else {
    # replicate columns to match n_lc
    rep_idx <- rep(seq_len(n_mc), length.out = n_lc)
    mc_paths_mean <- mc_paths_mean_all[, rep_idx, drop = FALSE]
}

# Now both are 52 x n_lc and can be added elementwise
adjusted_paths <- lc_means + mc_paths_mean # 52 x n_lc

# Lee-Carter forecast mean (2019) - use mean of the 10 LC forecasts
lc_forecast_mean <- rowMeans(lc_means, na.rm = TRUE)

# LSTM Monte Carlo paths mean (across age groups) - these are residuals, so add to Lee-Carter
mc_paths_mean <- apply(mc_paths, c(1, 3), mean, na.rm = TRUE) # 52 weeks x 10 paths
adjusted_mc_paths <- lc_forecast_mean + mc_paths_mean # Full forecast: Lee-Carter + LSTM residuals

# To improve readability, plot all 10 paths
adjusted_paths_subset <- adjusted_paths # 52 x 10

# Observed mean (2013-2019) - split into train (2013-2018) and test (2019)
observed_train <- observed_mean %>% filter(Week <= as.Date("2018-12-31"))
observed_test <- observed_mean %>% filter(Week > as.Date("2018-12-31"))

# Weeks for 2019 forecast
forecast_weeks <- lc_forecasts[[1]]$Week # Use weeks from first LC forecast

# Plot with improved readability
plot(
    observed_train$Week,
    observed_train$mean_logmxt,
    type = 'l',
    col = 'black',
    lwd = 3,
    xlab = 'Week',
    ylab = 'Mean Log Mortality Rate',
    main = 'Observed + Lee-Carter + LSTM Simulated Paths with Uncertainty (2019)',
    xlim = range(c(observed_train$Week, observed_test$Week)), # Up to 2019
    ylim = range(c(
        observed_train$mean_logmxt,
        observed_test$mean_logmxt,
        adjusted_paths_subset
    )),
    bty = 'n' # Remove box
)
grid() # Add grid for readability

# Add observed test (2019)
lines(observed_test$Week, observed_test$mean_logmxt, col = 'black', lwd = 3)


# LC PATH WITH UNCERTAINTY!
# Add adjusted LSTM simulated paths with LC uncertainty (all 10, less transparent, thicker)
matlines(
    forecast_weeks,
    adjusted_paths_subset,
    col = rgb(1, 0, 0, 0.3),
...
legend(
    'topright',
    legend = c(
        'Observed (Train: 2013-2018)',
        'Observed (Test: 2019)',
        'Lee-Carter + LSTM Simulated Paths (2019)'
    ),
    col = c('black', 'black', rgb(1, 0, 0, 0.5)),
    lwd = c(3, 3, 1.5),
    lty = c(1, 1, 1),
    bg = 'white'
)
...
# --- Plot for Single Age Group with Uncertainty ---
# Choose a specific age group
specific_age <- "67" # Change as needed

# Observed for specific age: mean across regions
age_cols <- grep(
    paste0("^", specific_age, "_"),
    names(observed_logmxt_wide),
    value = TRUE
)
observed_age <- observed_logmxt_wide %>%
    dplyr::select(Week, all_of(age_cols)) %>%
    dplyr::mutate(
        mean_logmxt = rowMeans(dplyr::select(., -Week), na.rm = TRUE)
    ) %>%
    dplyr::select(Week, mean_logmxt)

observed_age_train <- observed_age %>% filter(Week <= as.Date("2018-12-31"))
observed_age_test <- observed_age %>% filter(Week > as.Date("2018-12-31"))

# LC forecasts for specific age
lc_age_means <- matrix(NA, 52, 10)
for (i in 1:10) {
    age_cols_lc <- grep(
        paste0("^", specific_age, "_"),
        names(lc_forecasts[[i]]),
        value = TRUE
    )
    lc_age_means[, i] <- rowMeans(
        lc_forecasts[[i]][, age_cols_lc, drop = FALSE],
        na.rm = TRUE
    )
}

# LSTM MC for specific age - robust selection and fallback
age_idx <- integer(0)
if (!is.null(colnames(point_forecast))) {
    age_idx <- which(colnames(point_forecast) == specific_age)
}
if (length(age_idx) == 0 && !is.null(colnames(point_forecast))) {
    # try numeric coercion (some colnames may be numeric-like)
    suppressWarnings({
        cn_num <- as.character(as.numeric(colnames(point_forecast)))
    })
    age_idx <- which(cn_num == specific_age)
}
if (length(age_idx) == 0 && !is.null(colnames(point_forecast))) {
    # try prefix or leading-match (e.g. "AgeGroup_67" or "67_")
    age_idx <- which(grepl(
        paste0('^', specific_age, '($|_)'),
        colnames(point_forecast)
    ))
}
if (length(age_idx) == 0) {
    warning(sprintf(
        "Could not find age column '%s' in point_forecast; falling back to average across ages.",
        specific_age
    ))
    # average across age groups in mc_paths to create a surrogate for the requested age
    if (exists('mc_paths') && length(dim(mc_paths)) == 3) {
        mc_paths_age <- apply(mc_paths, c(1, 3), mean, na.rm = TRUE)
    } else {
        # construct a zero matrix with weeks matching lc_age_means and columns equal to 1 path
        mc_paths_age <- matrix(0, nrow = nrow(lc_age_means), ncol = 1)
    }
} else {
    # extract selection and coerce to a weeks x paths matrix
    mc_sub <- mc_paths[, age_idx, , drop = FALSE]
    if (length(dim(mc_sub)) == 3 && dim(mc_sub)[2] == 1) {
        mc_paths_age <- matrix(
            mc_sub[, 1, ],
            nrow = dim(mc_sub)[1],
            ncol = dim(mc_sub)[3]
        )
    } else if (length(dim(mc_sub)) == 3) {
        # multiple selected age columns -> average across selected ages
        mc_paths_age <- apply(mc_sub, c(1, 3), mean, na.rm = TRUE)
    } else {
        mc_paths_age <- as.matrix(mc_sub)
    }
}

# Adjusted paths for specific age: align dimensions before adding
if (!is.matrix(lc_age_means)) {
    lc_age_means <- as.matrix(lc_age_means)
}
if (!is.matrix(mc_paths_age)) {
    mc_paths_age <- as.matrix(mc_paths_age)
}

# Align rows (weeks)
nrow_lc_age <- nrow(lc_age_means)
nrow_mc_age <- nrow(mc_paths_age)
nmin_age <- min(nrow_lc_age, nrow_mc_age)
if (nrow_lc_age != nrow_mc_age) {
    warning(sprintf(
        "Row mismatch for age %s: LC=%d, MC=%d; truncating to %d rows.",
        specific_age,
        nrow_lc_age,
        nrow_mc_age,
        nmin_age
...
lc_age_sub <- lc_age_means[seq_len(nmin_age), , drop = FALSE]
mc_age_sub <- mc_paths_age[seq_len(nmin_age), , drop = FALSE]

# Align columns (n_lc sims vs n_mc paths)
ncol_lc_age <- ncol(lc_age_sub)
ncol_mc_age <- ncol(mc_age_sub)
if (ncol_lc_age == ncol_mc_age) {
    mc_age_mat <- mc_age_sub
} else if (ncol_mc_age > ncol_lc_age) {
    # subsample MC columns to match LC sims
    sel_cols <- round(seq(1, ncol_mc_age, length.out = ncol_lc_age))
    mc_age_mat <- mc_age_sub[, sel_cols, drop = FALSE]
} else {
    # replicate MC columns to reach LC sims
    rep_idx <- rep(seq_len(ncol_mc_age), length.out = ncol_lc_age)
    mc_age_mat <- mc_age_sub[, rep_idx, drop = FALSE]
}

# Now add
adjusted_paths_age <- lc_age_sub + mc_age_mat

# Plot all 10 paths
adjusted_paths_age_subset <- adjusted_paths_age

plot_file <- sprintf(
    'LC_LSTM_age_%s_%s.png',
    specific_age,
    format(Sys.Date(), '%Y%m%d')
)
png(filename = plot_file, width = 1400, height = 900, res = 150)

## Plot
plot(
    observed_age_train$Week,
    observed_age_train$mean_logmxt,
    type = 'l',
    col = 'black',
    lwd = 3,
    xlab = 'Week',
    ylab = paste('Log Mortality Rate for Age', specific_age),
    main = paste(
        'Observed + Lee-Carter + LSTM Simulated Paths for Age',
        specific_age,
        '(2019)'
    ),
    xlim = range(c(observed_age_train$Week, observed_age_test$Week)),
    ylim = range(c(
        observed_age_train$mean_logmxt,
        observed_age_test$mean_logmxt,
        adjusted_paths_age_subset
    )),
    bty = 'n'
)
grid()
...
legend(
    'topright',
    legend = c(
        paste('Observed (Train: 2013-2018)'),
        paste('Observed (Test: 2019)'),
        paste('LC + LSTM Paths (2019)')
    ),
    col = c('black', 'black', rgb(1, 0, 0, 0.5)),
    lwd = c(3, 3, 1.5),
    lty = c(1, 1, 1),
